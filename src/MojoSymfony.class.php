...
class MojoSymfony extends Mojo
{
  ...
  function Validate(){
    ...
    if(!MojoConfig::get('sf_mojo_dir') && isset($this->args['mojo_lib'])) MojoConfig::set('sf_mojo_dir',$this->args['mojo_lib']);
    if(!MojoConfig::get('sf_mojo_dir')) Mojo::exception("Your project directory is not defined. Specify it with --mojo_lib=./path/to/mojo/project/");
  }
...
#    print_r($rules); exit;
    ...
    Mojo::prompt("=======================================================================================");
    $written = MojoFile::write(MojoConfig::get('sf_mojo_dir').'rules/'.$name.'.js',self::printRule($rules,$form));
    if( $written > 0 ) Mojo::prompt("Rule written to: ".MojoConfig::get('sf_mojo_dir')."rules/".$name.".js");
    else Mojo::prompt("There was an error writting to :".MojoConfig::get('sf_mojo_dir')."rules/".$name.".js");

    $path = explode("/",$this->args['i18n']);
    $name = str_replace($path[count($path)-1],"",$this->args['i18n']);
    $file = str_replace("xml","js",$path[count($path)-1]);

    if(!file_exists(MojoConfig::get('sf_mojo_dir')."rules/i18n/")) mkdir(MojoConfig::get('sf_mojo_dir')."rules/i18n/"); 
    $written = MojoFile::write(MojoConfig::get('sf_mojo_dir')."rules/i18n/".$file,"var locData =".json_encode(self::printLocData($rules,$form)));
    if( $written > 0 ) Mojo::prompt("locData written to: ".MojoConfig::get('sf_mojo_dir')."rules/i18n/".$file);
    else Mojo::prompt("There was an error writting to :".MojoConfig::get('sf_mojo_dir')."rules/i18n/".$file);
  ...
  function printRule($rules,$form)
  {
      ...
      $str ="";
      $str .= MojoFile::editStream($this->args,self::Source('header'));
...
#        print_r($validators);
        ...
        if( count($validators) > 0){
          ...
          if($rule_count>1) $str .= MojoFile::editStream($this->args,self::Source('comma'));
          $str .= MojoFile::editStream($this->args,self::Source('field_start'));
          ...
          foreach($validators as $key => $value){
              ...
              switch($key){
                case 'required':
                  ...
                  $this->args['params'] = "";
                  $str .= MojoFile::editStream($this->args,self::Source('rule_start'));
                  $str .= MojoFile::editStream($this->args,self::Source('rule_end',$validators_end));
                ...
                case 'min': case 'min_length':
                  ...
                  $str .= MojoFile::editStream($this->args,self::Source('rule_start'));
                  $str .= MojoFile::editStream($this->args,self::Source('min_max'));
                  $str .= MojoFile::editStream($this->args,self::Source('rule_end',$validators_end));
                ...
                case 'invalid': case 'match':
                    switch(strtolower($value['key'])){
                      case 'confirm_email_match': case 'confirm_email': case 'confirm_password': case 'match':                                                 
                        ...
                        $str .= MojoFile::editStream($this->args,self::Source('rule_start'));
                        $str .= MojoFile::editStream($this->args,self::Source('match'));
                        $str .= MojoFile::editStream($this->args,self::Source('rule_end',$validators_end));
                      ...
                      case 'email_invalid':
                        ...
                        $str .= MojoFile::editStream($this->args,self::Source('rule_start'));
                        $str .= MojoFile::editStream($this->args,self::Source('regex'));
                        $str .= MojoFile::editStream($this->args,self::Source('rule_end',$validators_end));
          ...
          $str .= MojoFile::editStream($this->args,self::Source('field_end',$rule_end));
        }
        $rule_count++;
      }

      $str .= MojoFile::editStream($this->args,self::Source('footer'));
      return $str;
  }
